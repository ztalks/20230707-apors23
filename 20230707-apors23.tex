...
\tikzstyle{blackbox} = [rectangle, draw, fill=black, text width=3em,
text centered, rounded corners, minimum height=1.6em]
\tikzstyle{none} = [node distance=2cm, minimum height=1.6em]
\tikzstyle{line} = [draw, -latex]

\begin{center}
    \begin{tikzpicture}[node distance = 1.6em, auto]
        \node [blackbox] (f) {\textcolor{white}{$f$}};
          \node [none, left of=f] (x) {$x$};
          \node [none, right of=f] (fx) {$f(x)$};
          \path [line] (x) -- (f);
          \path [line] (f) -- (fx);
    \end{tikzpicture}
\end{center}
...
\begin{frame}
    \frametitle{An example --- hyperparameter tuning}
...
%        \vspace{1ex}
%   算法例子: Nealder-Mead 单纯形法, Powell 基于插值模型的方法
...
%      \item Engineers use derivative-free optimization methods sometimes
%        because they simply \red{do not want to} calculate derivatives
...
%    \begin{itemize}
%{
%\item Colson, \etal, Optimization methods for advanced design of \blue{aircraft panels}: a comparison, 2010.
%  \vspace{1ex}

%\item Ciccazzo, \etal, Derivative-free robust optimization for \blue{circuit design}, 2015
%\vspace{1ex}

%\item Wild, Sarich, and ~Schunck, Derivative-free optimization for parameter estimation in \blue{computational nuclear physics}, 2015
%\vspace{1ex}

%\item Campana, \etal, Derivative-free global \blue{ship design} optimization using global/local hybridization of the direct algorithm, 2016
%\vspace{1ex}

%\item Ghanbari and Scheinberg,
%  Black-box optimization in \red{machine learning} with trust region
%  based derivative free algorithm, 2017
%}

%    \end{itemize}
%%\pause
%%\vspace{1ex}
%%\item Well-developed theory and methods
%%\vspace{1ex}
%%{\small
%%\\- A. Conn, K. Scheinberg, L. N. Vicente,
%%\textit{Introduction~to~Derivative-free\\\hspace*{2mm}Optimization}, 2007}
%\end{frame}

%\begin{frame}
%    \frametitle{``Important and cool''}
%    \begin{center}
%    \includegraphics[width=0.5\textwidth]{Dennis.jpg}
%    \end{center}
%    %\vspace{3ex}
%    \begin{beamerboxesrounded}[width=12.08cm,shadow=true]{}
%        - Why work on derivative-free optimization? \\
%        - Because the problems are \textcolor{blue}{\mbox{important} and cool}.
%    \end{beamerboxesrounded}
%    \begin{flushright}
%        --- J. E. Dennis, Jr.
%    \vspace{0.3ex}
%        \small{\\ Reasons to study derivative-free algorithms, 2013,
%        Toulouse}
%    \end{flushright}
%\end{frame}

%\begin{frame}
%  \frametitle{No applications by Powell, because ...}
%  \vspace{2ex}
%    \begin{beamerboxesrounded}[width=12.08cm,shadow=true]{}
%      \small
%      The development of algorithms for optimization has been my main
%      field of research for 45 years, but \blue{I have given hardly any
%      attention to applications}. It is very
%      helpful, however, to try to solve some particular problems well,
%      in order to receive
%      guidance from numerical results, and in order \blue{not to be misled from efficiency in
%      practice by a desire to prove convergence theorems}.
%      ...
%      I was told  ... that the DFP algorithm (Fletcher and Powell,
%      1963) had assisted the \red{moon landings of the Apollo 11 Space Mission}.
%    \end{beamerboxesrounded}
%    \vspace{-0.3ex}
%    \begin{flushright}
%      --- \small{Powell}
   ...
   \vspace{1ex}
    ...
    \begin{itemize}
        \item \blue{Model-based} methods: \\
          ...
          %in adaptively chosen \blue{trust regions}.
          \vspace{1ex}

          \begin{itemize}
              \item Powell's \blue{trust-region} DFO methods\\[1ex]
            \item \blue{SOLNP} and \blue{SOLNP+} by Ye \textit{et al}% (\blue{NEWUOA}, \blue{COBYLA}, etc.)
    %are {trust-region} method based on \textcolor{blue}{linear or quadratic models} built by \textcolor{blue}{interpolation}.
          \end{itemize}
 \vspace{1ex}

 \item \blue{Direct search} methods: \\
     iterates are determined by \blue{simple comparison} of function values% %without building models.

   \vspace{1ex}

   \begin{itemize}
       \item Nelder-Mead \blue{simplex method}
       %, {Implicit Filtering} (Gilmore and Kelley, 1995), \texttt{GPS} (Torczon, 1997), \texttt{MADS} (Audet and Dennis, 2006), \texttt{BFO} (Porcelli and Toint, 2015), \dots
   \end{itemize}
    \end{itemize}
